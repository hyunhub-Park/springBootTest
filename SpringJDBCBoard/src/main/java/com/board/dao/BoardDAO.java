...
public class BoardDAO
{
	...
	public static String list = "SELECT board_no, title, content, writer, reg_date FROM jdbcBoard WHERE board_no > 0 ORDER BY board_no desc, reg_date DESC";
	public static String selectOne = "SELECT board_no, title, content, writer, reg_date FROM\r\n"
			+ "jdbcBoard WHERE board_no = ?";
	...
	{
		return jdbcTemplate.update(modify, board.getTitle(),
									board.getContent(),
									board.getBoardNo());
	}
}